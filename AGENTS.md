...
Backend fixes (Task B, commit `88c50e6`): removed the dead existing-voice-note pre-check in the voice-note submission path; event-close now maps `ARCHIVED` to `INVALID_EVENT_STATE`; non-UUID `media_id` returns `404`; server-side audio validation rejects video-container audio.

Audit remediation wave (2026-08-28, commits `c7d561f`/`f7ffb2f`/`209eb26`, pushed `3698745..209eb26`): full 4-lane audit (docs/code/UI/cross-check) then fixes. `c7d561f` docs sync — all `docs/UI_UX.md`/`UI_DESIGN.md` dangling refs removed (repointed to DESIGN.md root), PRD §5.2 flow +FRAME_SELECT step, DESIGN.md §5.6 Done receipt copy → Bahasa Indonesia (mirrors Done.tsx), UX_FLOW.md §2 CTA copy → "Pakai {Frame}"/"Tanpa Frame, lanjut", legacy frame-selector inventory row removed, §12 frame-count detail removed (count intentionally unrecorded — changes over time). `209eb26` UI fixes — PreSession Shell safe-area bottom (`pb-[calc(2rem+env(safe-area-inset-bottom))]`); input border-color transitions removed (PreSession + admin-input, §4 never-animates); `--motion-develop: 800ms` token ratified (globals.css + DESIGN.md §4, documents existing `animate-develop`); admin event-index row Open demoted to secondary (Create Event = sole gold primary per §6); PhotoReview counter → `font-mono tabular-nums`; FrameSelection indicator dots → scaleX transform (no width animation). Deferred LOW findings (owner decision pending): case-sensitive guest_name search, API-level sign-in rate limiting, PHOTO_LIMIT dedup, isConstraintViolation hardening, signed-URL clock drift, unguarded smooth scrollIntoView, guest/admin error-color inconsistency.

Live Supabase integration verified: live PostgreSQL schema tests PASS, `PLAYWRIGHT_LIVE=1` PASS, seeded events present (1 ACTIVE). Live DB migration state verified: `supabase_migrations.schema_migrations` records `0001`–`0008` (per earlier live verification; not re-verified here). Repo migration files present: `0001`–`0004`, `0007`–`0009` (`0005`/`0006` absent from repo); `0009_drop_guest_messages.sql` drops the unused guest-messages table and is not reflected in the recorded range above. `guest_sessions.public_ref` applied — NOT NULL, unique index `uq_guest_sessions_public_ref`, 0 NULL/duplicate values; `expires_at` present with 30-minute default. Migration `0007` (2026-08-17): pins explicit service_role table grants — photos/voice_notes SELECT+DELETE, events SELECT+INSERT+UPDATE, guest_sessions SELECT+INSERT; no grants for admins/session_create_rate_limits (no service_role usage). Migration `0008` (2026-08-17): guest-media `storage.objects` RLS policies — service_role SELECT/INSERT/DELETE, applied manually via dashboard (3 policies); repo file is documentation-only. All migrations are idempotent; migration 0002 sets `search_path = public, extensions` for pgcrypto resolution on Supabase.

Indonesian UI sweep + admin sign-in redesign (2026-08-29, T043/T044): all user-facing copy (guest + admin, visible text + aria-labels + sr-only + placeholders) converted to Bahasa Indonesia, casual guest-page tone throughout — owner-approved reversal of the 2026-08-22 English admin labels decision and the English aria-label convention. `app/layout.tsx` `lang="id"`. e2e specs (all 5) + `guest-flow-perf.mjs` harness + `component-catalog.html` synced in-commit (stale Done entry #15 replaced with thermal-print implementation; sign-in entry #25 re-rendered per redesign). Server/API error messages remain English (API contract domain). Admin sign-in redesigned (T044, designer lane): asymmetric editorial split (`lg:grid-cols-[1.1fr_1fr]`, offset form column), copy contract "Akses admin"/"Kelola acaramu."/"Buat acara, bagikan akses, dan lihat semua kiriman."/"Masuk"/"Sebentar, ya…"; DESIGN.md §6 sign-in spec synced. Behavior/API/schema unchanged; auth flow, error mapping logic untouched (one fix: `error.toLowerCase().includes("udah ada")` gates the ACTIVE_EVENT_EXISTS recovery link — was case-sensitive and broke against the translated message). Known-fragile pattern: client logic keying off message substrings — future copy changes must re-check `admin-ui.tsx` create-form link gating. T041 hardening fixes (mic/voice-timer unmount teardown, syncPhotos mutex, start() guard, capture fail-soft, object-URL/XHR cleanup) committed `dc12c73`; DESIGN.md §5.6 thermal-print spec sync `7fa31f0`; T042 e2e Done-arrival assertion fix `0174aae`. Full validation post-sweep: typecheck PASS, vitest 374/374, build PASS, e2e 37 passed / 1 skipped (live-backend, expected).

Local QA: `npx vitest run` 373/373 (46 files); Typecheck PASS. Playwright E2E full run 37 passed / 1 skipped (2026-08-28, post `209eb26`; skip = live-backend test, expected without `PLAYWRIGHT_LIVE=1`); mobile-media-qa 19/19 PASS (2026-08-21, post 3-zone capture redesign). Lint has pre-existing issues only (1 `any` error in `e2e/print-qa.spec.ts`, 12 warnings — all pre-existing drift on HEAD-identical code; tally corrected from 11 during the 2026-08-22 session close, `next-env.d.ts` excluded). Chromium is installed.
