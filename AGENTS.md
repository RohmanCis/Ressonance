...
Local QA: `npx vitest run` 344/344 (43 files) PASS; Typecheck PASS (2026-08-20, post ADR-012 deepening; feature removal dropped 4 test files and 51 tests). Playwright `smoke+qr-qa+print-qa` 11 passed / 1 skipped / 0 failed were verified before the sequential-flow refactor. Lint has pre-existing issues only (1 `any` error in `e2e/print-qa.spec.ts`, 7 warnings, `next-env.d.ts` excluded). Chromium is installed.
