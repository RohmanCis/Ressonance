...
Local QA: `npx vitest run` 395/395 (47 files) PASS; Typecheck PASS (2026-08-20, post ADR-012 deepening: baseline was 375/41 before the shared guest-submission seam + usage types; +17 auth/pipeline/payload tests, +3 usage tests, −18 auth tests moved out of submit suites). Playwright `smoke+qr-qa+print-qa` 11 passed / 1 skipped / 0 failed were verified before the sequential-flow refactor. Lint has pre-existing issues only (1 `any` error in `e2e/print-qa.spec.ts`, 7 warnings, `next-env.d.ts` excluded). Chromium is installed.
