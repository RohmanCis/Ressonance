...
export function AdminCreateEvent() {
  ...
  return <AuthGate><Shell><div className="mx-auto max-w-2xl"><p className="text-xs font-medium tracking-[0.04em] text-text-muted">New event</p><h1 className="mt-2 font-display text-3xl font-semibold tracking-tight text-text-primary">Create a new event.</h1><p className="mt-3 text-text-secondary">Create one active event. You can close it when the day is complete.</p><form onSubmit={submit} className="mt-8 rounded-[10px] border border-border bg-bg-surface p-6"><label className="block text-xs font-medium text-text-secondary" htmlFor="event-title">Event title<input id="event-title" required value={title} onChange={e => setTitle(e.target.value)} className="mt-2 h-11 w-full rounded-md border border-border bg-bg-base px-3 text-text-primary placeholder:text-text-muted focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent" placeholder="Summer party" /></label>{error && <Status error message={error} action={error.includes("already") ? <Link href="/admin" className="underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent">Find existing event</Link> : undefined} />}{message && <Status message={message} />}<Button disabled={busy} className="mt-6">{busy ? "Creating…" : "Create event"}</Button></form></div></Shell></AuthGate>;
}
