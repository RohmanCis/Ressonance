...
- **Layout:** Centered, full-screen `--bg-base`. Disposable-camera photo-eject sequence (T034, owner spec 2026-08-29), five timed phases on mount: (1) DM Mono loading text "Sebentar ya, foto kamu lagi diproses" with staggered dots; (2) inline token-colored disposable-camera illustration drops in (camera-drop, transform+opacity only); (3) the keepsake ejects from the film slot (photo-eject) — a 140px 9:16 composited photo when one exists (Condition A), or a Mic chip "Pesan suara tersimpan" for voice-only sessions (Condition B); (4) thank-you — gold check glyph (not animated), event title in Cormorant Garamond 4xl (heading receives focus on arrival), and two short lines of Bahasa-Indonesia receipt copy ("Terima kasih — foto dan pesan suara kamu sudah kami terima." and "Host akan melihatnya setelah acara."); (5) optional keepsake card. Condition C (nothing confirmed) skips the camera and shows the thank-you directly. `prefers-reduced-motion` skips to the final phase immediately with all content visible (§4).
- **Key visual elements:** No actions, no navigation, no submission affordances. Exception: a keepsake download card is permitted as the sole optional action on the Done screen. It must use bg-bg-surface, border-border, no gold accent, and subdued text-text-secondary styling.
