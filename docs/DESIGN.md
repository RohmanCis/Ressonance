...
- **Key visual elements:** Heading "Foto Pilihanmu" with DM Mono gold counter "(N/5)"; helper "Kamu bisa memotret ulang, menghapus, atau lanjut simpan." Photo tiles in a 3-column grid on `--bg-surface` with DM Mono "#N" labels; per-item status badges (pending / uploading spinner / confirmed check / error) with delete and retry controls. Gallery header row: "GALERI SESI INI" + quota hint ("Kuota Penuh" / "Sisa N Foto"). Empty state: "Belum ada foto yang dijepret di sesi ini."
...
- **Layout:** Full-screen `--bg-base`, safe-area padded. Event title script eyebrow + Cormorant Garamond 3xl heading ("Tinggalkan Pesan Suara") + helper ("Ungkapkan doa & ucapan hangat untuk kedua mempelai secara personal.").
- **Center Stage:** Gold mic button (h-20 w-20; idle = gold fill, recording = `--bg-surface` with gold border + gold glow), DM Mono timer (00:00 / 00:30), and a decorative 9-bar equalizer (`aria-hidden`) that animates with `animate-pulse` while recording (§2 amendment). A soft `animate-pulse` halo rings the mic while recording; a pulsing gold dot marks the live status line ("Tahan berbicara… (Ns lagi)" below 5s, else "Sedang merekam… ketuk kotak jika selesai"). Mic toggle fires `navigator.vibrate(40)` (§4).
...
- **Key visual elements:**
  ...
  - **Thank-you block (phase ≥ 4):** Event title is `sr-only` — not visually shown; rendered as an `<h1>` (`tabIndex={-1}`, focus moved to it on arrival) for heading semantics only. Two receipt lines: "Terima kasih — foto dan pesan suara kamu sudah kami terima." (`text-sm text-text-secondary`) and "Host akan melihatnya setelah acara." (`text-sm text-text-muted`). No gold check glyph, no visible Cormorant event title. No actions, no navigation, no submission affordances except the optional keepsake card below.
