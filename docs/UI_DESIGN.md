...
Use a paper page frame with event title at the top, a compact context line below, and the active task in one stacked content column. Pre-session shows title, optional name, and Start; no submission affordance appears before successful Start. Post-Start places photo and voice actions as two separated blocks, followed by server-returned usage such as `Photos remaining: 3/5` and `Voice note: Available` or its confirmed consumed state.

Loading uses reserved skeleton blocks. Closed, limit, session-invalid/expired, offline, rate-limited, and unexpected states use the status/alert surface and disable only affected actions as a hint. Do not decide expiry wording or retention messaging here. Submission progress stays inside the media block; success replaces it with confirmation and updated usage, preventing layout jumps.
...
Photo selection offers camera capture and file selection where supported. Permission request precedes the browser prompt. Denied/unsupported state keeps a file-selection fallback when available. Selected/captured state uses a fixed-ratio review tile (`1:1` thumbnail, `4:3` preview where source allows), `object-fit: cover` for tile previews, and visible remove/replace controls. Submission adds per-item progress; success confirms persistence; failure stays attached to the item with retry or correction.
