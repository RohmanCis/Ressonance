...
**Amended 2026-08-17 (frame pipeline normalization, owner decision):** §11 adds the frame-selection card grid and the 9:16 treatment for the viewfinder, frame previews, and the frame overlay (`object-fit: contain`, never mirrored or stretched). Camera captures are composited to a fixed 1080×1920 via center cover-crop. Frame artwork is placeholder and replaceable without code changes; the structural invariants (1080×1920, true alpha, transparent photo area) are the contract.
...
Pre-session uses a paper page frame with event title at the top, a compact context line below, optional name field, and Start action in one stacked content column. No submission affordance appears before successful Start.

**Post-Start sequential flow (amendment 2026-08-20):** The post-Start guest experience is a sequence of distinct full-screen states (UI_UX §4.3–§4.7): capture → photo review → voice → done.

**Capture screen:** The camera viewfinder is a fullscreen layer (minus safe areas) with event title and guest name in a compact top bar, shutter in the lower thumb zone with `env(safe-area-inset-bottom)` clearance, a remaining-photo indicator near the shutter, a pending photo strip as a bottom overlay row, and a "Lanjut" advance action visible without scroll. The remaining-photo counter sits bottom-right near the shutter. Photo synchronization does not occur on this screen; it moves to the photo review screen (UI_UX §4.4).
...
Photo capture uses a camera-first viewfinder surface (`getUserMedia` live preview, `playsInline`, `muted`, `autoPlay`, `object-fit: cover`). The viewfinder and all frame surfaces use the enforced 9:16 ratio: every camera capture is composited to a fixed 1080×1920 output via deterministic center cover-crop (UI_UX §4.5), so the preview is WYSIWYG on every sensor. When a frame is active, its overlay sits above the live preview with `object-fit: contain` — never mirrored, never stretched — and photo area transparency comes from the frame asset's true alpha channel. Permission request precedes the browser prompt. Denied/unsupported state keeps a file-selection fallback when available; the pending strip and remaining indicator remain available for any captures already taken. File-picker uploads are intentionally unframed and are not normalized to 9:16. The shutter button sits in the lower thumb zone, 48px+ height, `env(safe-area-inset-bottom)` clearance, with capture feedback (flash overlay). A camera switch toggle is shown only when `enumerateDevices` reports two or more cameras.

Frame selection (UI_UX §4.2) is a card grid in the guest shell: one card per frame option, each a 9:16 preview container showing the frame artwork with `object-fit: contain` (never distorted), a selected state using border-primary + ring-primary, and a check badge on the selected card. Cards operate as a radio group (arrow-key navigation, roving tabindex, visible focus, non-visual checked state). "No Frame" is never a card; skip/continue affordances sit below the grid, primary confirm full-width per guest CTA rules.

A pending photo strip shows horizontal scrollable thumbnails (~48px) of captured-but-unsent and server-confirmed items. Each thumbnail carries a per-item status indicator: `pending`, `uploading` (spinner), `confirmed` (check), `error` (error icon + code), or `expired` (not saved). Tapping a pending thumbnail opens a full-size review overlay with delete and retake controls (Level 2 elevation, `--scrim` backdrop, `1:1` thumbnail ratio, `9:16` preview for camera captures where source allows, `object-fit: cover`). Confirmed items cannot be deleted — no delete endpoint exists. The remaining-photo indicator uses tabular figures and `aria-live` polite, placed near the shutter.

Batch synchronization — triggered from the photo review screen's primary CTA (UI_UX §4.4), not a Send action on the capture screen — shows aggregate progress with per-item fidelity: each thumbnail transitions through spinner → check (201 confirmed) or error icon (rejected). On sync complete, a summary of accepted and rejected items is shown. `RATE_LIMITED` (429) pauses the queue and honors `Retry-After`. `PHOTO_LIMIT_REACHED` (409) marks the item as error and frees the local budget. `SESSION_EXPIRED` or `SESSION_INVALID` (401) discards session authority; pending items remain visible marked as not saved. `EVENT_CLOSED` (422) marks the item as error with no retry.
